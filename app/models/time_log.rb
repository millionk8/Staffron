class TimeLog < ActiveRecord::Base
  ...
  def check_overlap
    time_logs_count = TimeLog.where('user_id = :user_id AND (started_at BETWEEN :start AND :stop) OR (stopped_at BETWEEN :start AND :stop) OR (started_at <= :start AND stopped_at >= :stop)', user_id: user.id, start: started_at, stop: stopped_at).count
